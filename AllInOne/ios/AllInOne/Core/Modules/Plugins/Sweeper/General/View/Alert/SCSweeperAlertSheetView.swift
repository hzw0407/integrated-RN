...
typealias SCSweeperAlertSheetViewBlock = (_ index: Int) -> Void
private let kHorizontalMargin: CGFloat = 20

class SCSweeperAlertSheetView {
    private static let sharedInstance = SCSweeperAlertSheetView()
    ...
    fileprivate var actionBlock: SCSweeperAlertSheetViewBlock?
    ...
    static func alert(title: String? = tempLocalize("提示"), message: String, supplement: String? = nil, actionsTitles: [String], callback: SCSweeperAlertSheetViewBlock?) {
        let `self` = SCSweeperAlertSheetView.sharedInstance
